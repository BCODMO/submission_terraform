...
[
    {
        ...
        "environment": [
            ...
            { "name": "MINIO_ENDPOINT", "value": "${var.aws_endpoint}" },
            { "name": "MINIO_ACCESS_KEY", "value": "${var.aws_access_key}" },
            { "name": "MINIO_SECRET_KEY", "value": "${var.aws_secret_access_key}" },
            { "name": "MINIO_USE_SSL", "value": "false" },
            { "name": "MINIO_SUBMISSIONS_BUCKET", "value": "${aws_s3_bucket.submissions.bucket}" },
            { "name": "MINIO_PROJECTS_BUCKET", "value": "${aws_s3_bucket.projects.bucket}" },
            { "name": "MINIO_PERMISSIONS_BUCKET", "value": "${aws_s3_bucket.submissions_permissions.bucket}" },
            { "name": "OFFLINE_DEVELOPMENT", "value": "true" },
            { "name": "SUBMISSION_VALIDATION_URL", "value": "" },
            { "name": "SUBMISSION_VALIDATION_REQUEST_API_KEY", "value": "${var.api_request_key}" },
            { "name": "SUBMISSION_VALIDATION_RESPONSE_API_KEY", "value": "${var.api_response_key}" },
            { "name": "ID_GENERATOR_URL", "value": "${var.id_generator_url}" },
            { "name": "ID_GENERATOR_API_KEY", "value": "${var.id_generator_api_key}" },
            { "name": "AUTH_FORCE_LOGOUT_URL", "value": "${var.auth_force_logout_url}" },
            { "name": "AUTH_URL", "value": "${var.auth_url}" },
            { "name": "AUTH_TOKEN_URL", "value": "${var.auth_token_url}" },
            { "name": "AUTH_CLIENT_ID", "value": "${var.auth_client_id}" },
            { "name": "AUTH_CLIENT_SECRET", "value": "${var.auth_client_secret}" },
            { "name": "ORCID_API_URL", "value": "${var.orcid_api_url}" },
            { "name": "PORT", "value": "80" }
        ]
    }
]
